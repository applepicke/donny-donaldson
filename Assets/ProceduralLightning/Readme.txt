...
Tutorial Videos:
Getting Started: https://youtu.be/3eJEKfWBlhs
...
- Prefabs (/Prefab)
	...
	- LightningBoltPrefab - Use this for lightning that has just a start and end point
...
*Orthographic Camera*
Procedural Lightning works a little differently in orthographic mode and is more efficient in generating the lightning and rendering it with the shader. If you are using an orthographic camera with a depth axis other than Z, you will need to change a few things to get the lightning to look right, namely the shader and generator will need to not assume that z is the depth axis.
